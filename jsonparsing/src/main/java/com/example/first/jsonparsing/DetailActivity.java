...
public class DetailActivity extends AppCompatActivity {
    ...
    Handler handler = new Handler(){
        ...
        public void handleMessage(Message msg) {
            if(msg.what == 0){
                ...
                // 파일 이름을 가지고 이미지를 다운로드 받는 Thread 생성 및 시작
                ImgThread th = new ImgThread(map.get("pictureurl"));
                th.start();
            }else if(msg.what == 1){
                // Message의 obj를 ImageView에 출력
    ...
    class ThreadEx extends Thread {
        ...
        public void run() {
            ...
            try {
                // 다운로드 받을 주소 생성
                URL url = new URL("http://192.168.0.229:8015/androidphone/getitem?itemid=" + itemid);
                // Connection 생성
    ...
    // 파일 이름을 가지고 이미지를 다운로드 받는 Thread
    class ImgThread extends Thread{
       ...
       public ImgThread(String filename){
        ...
        public void run() {
           try{
            InputStream is = new URL("http://192.168.0.229:8015/androidphone/img/" + filename).openStream();
